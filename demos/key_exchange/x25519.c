...
int x25519_key_exchange()
{
    ...
    // Create Alices key
    if (cw_keyexch_x25519_init(&alice, NULL, 0) != 1)
    {
    ...
    // Create Bobs key
    if (cw_keyexch_x25519_init(&bob, NULL, 0) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
    ...
    if (cw_keyexch_x25519_derive(alice, bob_pub_key, bob_pub_key_len, &alice_secret, &alice_secret_len, 0) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
    ...
    if (cw_keyexch_x25519_derive(bob, alice_pub_key, alice_pub_key_len, &bob_secret, &bob_secret_len, 0) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
