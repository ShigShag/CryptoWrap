...
int x448_key_exchange()
{
    ...
    // Create Alices key
    if (cw_keyexch_dhec_init(&alice, CW_NIST_P_512) != 1)
    {
    ...
    // Create Bobs key
    if (cw_keyexch_dhec_init(&bob, CW_NIST_P_512) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
    ...
    if (cw_keyexch_dhec_derive(alice, bob_pub_key, bob_pub_key_len, &alice_secret, &alice_secret_len, 0) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
    ...
    if (cw_keyexch_dhec_derive(bob, alice_pub_key, alice_pub_key_len, &bob_secret, &bob_secret_len, 0) != 1)
    {
        cw_error_get_last_error_fp_ex(stdout);
        goto END;
    }
